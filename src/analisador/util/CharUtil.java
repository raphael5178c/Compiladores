...
public class CharUtil {
	...
	public static char getNextChar(char[] charArray, int currentPos) {
		try {
			if (ValueUtil.isEmpty(charArray) || ValueUtil.isEmpty(currentPos) || (currentPos > charArray.length)
					|| ((currentPos + 1 > charArray.length)) || currentPos < 0)
				return 0;
	...
	public static String charToString(char character) {
		if (ValueUtil.isEmpty(character))
			return null;
		return character + "";
	}
